...
/**
 * Flat Pricing Model Analysis Script
 * 
 * Queries MongoDB to find all records using "flat" pricingModel
 * These should likely be "monthly" instead for ongoing monthly services.
 * 
 * Run with: npx tsx scripts/countFlatPricingModel.ts
 */
...
interface FlatPricingStats {
  totalPublications: number;
  publicationsWithFlat: number;
  totalFlatRecords: number;
...
async function countFlatPricingModel() {
  console.log('\n' + '='.repeat(80));
  console.log('  🔍 FLAT PRICING MODEL ANALYSIS');
  console.log('  Finding records with pricingModel: "flat" instead of "monthly"');
  ...
  try {
    ...
    const stats: FlatPricingStats = {
      totalPublications: publications.length,
      publicationsWithFlat: 0,
      totalFlatRecords: 0,
      byChannel: {},
      details: []
    };
    ...
    for (const pub of publications) {
      ...
      if (channels.website?.advertisingOpportunities) {
        channels.website.advertisingOpportunities.forEach((ad: any, idx: number) => {
          if (ad.pricing?.pricingModel === 'flat') {
            pubsWithFlat.add(pubId);
            stats.totalFlatRecords++;
        ...
        channels.print.forEach((print: any, printIdx: number) => {
          print.advertisingOpportunities?.forEach((ad: any, adIdx: number) => {
            ...
            pricingArray.forEach((pricing: any, tierIdx: number) => {
              ...
              if (pricingObj?.pricingModel === 'flat') {
                pubsWithFlat.add(pubId);
                stats.totalFlatRecords++;
    ...
    stats.publicationsWithFlat = pubsWithFlat.size;
    ...
    console.log(`Publications with "flat" pricingModel: ${stats.publicationsWithFlat}`);
    console.log(`Total Records with "flat" pricingModel: ${stats.totalFlatRecords}\n`);

    if (stats.totalFlatRecords > 0) {
      ...
      console.log('✅ No records found with "flat" pricingModel!\n');
    }

    console.log('=' .repeat(80));
    console.log('💡 RECOMMENDATION:');
    console.log('='.repeat(80));
    console.log('These records should likely use pricingModel: "monthly" instead of "flat"');
    console.log('The "flat" pricing model is typically used for event-based pricing with frequency.');
    console.log('For ongoing monthly services (website, newsletter), use "monthly" instead.');
    console.log('='.repeat(80) + '\n');

    // Save detailed report
    const fs = await import('fs');
    const reportPath = `reports/flat-pricing-analysis-${new Date().toISOString().replace(/:/g, '-')}.json`;
