import { connectToDatabase, getDatabase } from '../integrations/mongodb/client.js';
...
async function addSampleSurvey() {
  try {
    ...
    const collection = db.collection(COLLECTIONS.SURVEY_SUBMISSIONS);
    
    const sampleSurvey = {
      metadata: {
        respondentId: Date.now().toString(),
        collectorId: "web-form-sample",
        startDate: new Date("2025-10-03T19:00:00.000Z"),
        endDate: new Date("2025-10-03T19:15:30.000Z"),
    ...
    const result = await collection.insertOne(sampleSurvey);
    console.log('✅ Sample survey added successfully!');
    console.log('📄 Inserted document ID:', result.insertedId);
    console.log('🏢 Company:', sampleSurvey.contactInformation.companyName);
    console.log('📧 Contact:', sampleSurvey.contactInformation.email);
    console.log('📰 Outlets:', sampleSurvey.contactInformation.mediaOutletNames);
    
    // Verify the insertion
    const count = await collection.countDocuments();
    console.log('📊 Total survey submissions in database:', count);
    
  } catch (error) {
    console.error('❌ Error adding sample survey:', error);
  }
}
