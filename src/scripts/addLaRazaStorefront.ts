...
import { createStorefrontConfiguration } from '../integrations/mongodb/storefrontService';
...
async function addLaRazaStorefront() {
  try {
    console.log('🔄 Adding La Raza Chicago storefront configuration...');
    
    const result = await createStorefrontConfiguration(laRazaStorefrontConfig);
    
    console.log('✅ Successfully added La Raza Chicago storefront configuration:');
    console.log(`   - ID: ${result._id}`);
    console.log(`   - Publisher ID: ${result.meta.publisher_id}`);
    console.log(`   - Publication ID: ${result.publicationId}`);
    console.log(`   - Components: ${Object.keys(result.components).length}`);
    console.log(`   - Enabled Components: ${Object.values(result.components).filter(c => c.enabled).length}`);
    console.log(`   - Status: ${result.meta.is_draft ? 'Draft' : 'Published'}`);
    
    return result;
  } catch (error) {
    console.error('❌ Error adding La Raza storefront configuration:', error);
    throw error;
  }
}
...
export { addLaRazaStorefront, laRazaStorefrontConfig };
